...
class InputLayout(object):
    ...
    def encode(self, vertex, vbuf_idx, stride, index = 0):
        buf = bytearray(stride)
        # filename = 'D:/Naraka/program/Mods/all/export-vb0.txt'
        # with open(filename, 'a') as vbTxt:
        #     vbTxt.write('\n')
        ...
        for semantic, data in vertex.items():
            ...
            # with open(filename, 'a') as vbTxt:
            #     vbTxt.write(f'vb[{index}] {semantic}: {dataStr}\n')
        ...
        for vbuf_idx, stride in strides.items():
            with open(output_prefix + vbuf_idx, 'wb') as output:
                for index, vertex in enumerate(self.vertices):
                    output.write(self.layout.encode(vertex, vbuf_idx, stride, index))
...
class IndexBuffer(object):
    ...
    def write(self, output, operator=None):
        ...
        # filename = 'D:/Naraka/program/Mods/all/export-ib.txt'

        # with open(filename, 'a') as ibText:
        #     for face in self.faces:
        #         output.write(self.encoder(face))
        #
        #         faceStr = ' '.join(map(str, face))
        #         ibText.write(f'{faceStr}\n')
...
def import_3dmigoto_vb_ib(operator, context, paths, flip_texcoord_v=True, axis_forward='-Y', axis_up='Z', pose_cb_off=[0,0], pose_cb_step=1):
